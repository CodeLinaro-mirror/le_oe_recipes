...
FILES_${PN}-dbg += "/usr/lib/hw/.debug"
...
do_install_append() {
    ...
    install -d ${D}${sysconfdir}/data/misc/bluedroid/

    install -d ${D}${sysconfdir}/bluetooth/

    install -d ${D}/usr/lib/hw/

    install -m 0664 ${D}/usr/lib/libbluetoothdefault.so.0 ${D}/usr/lib/hw/
    ...
    if [ -f ${S}conf/iot_devlist.conf ]; then
       ...
       install -m 0660 ${S}conf/iot_devlist.conf ${D}${sysconfdir}/data/misc/bluedroid/
    fi
}
