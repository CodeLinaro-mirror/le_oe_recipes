inherit module autotools

DESCRIPTION = "Neutrino2 Ethernet driver"
LICENSE = "GPL-2.0"
LIC_FILES_CHKSUM = "file://${COREBASE}/meta/files/common-licenses/GPL-2.0;md5=801f80980d171dd6425610833a22dbe6"

FILES_${PN}     += "${base_libdir}/modules/${KERNEL_VERSION}/"
FILES_${PN}     += "${sysconfdir}/init.d/neutrino2_start_stop_le"
FILES_${PN}     += "${sysconfdir}/init.d/setup_avtp_routing_le"
RPROVIDES_${PN} += "kernel-module-tc9562-pcie-eth"

do_unpack[deptask] = "do_populate_sysroot"
PR = "r0-${KERNEL_VERSION}"

FILESPATH =+ "${WORKSPACE}:"
SRC_URI = "file://qcom-opensource/ethernet/neutrino2/standalone/driver/"
SRC_URI += "file://neutrino2_start_stop_le"
SRC_URI += "file://setup_avtp_routing_le"

S = "${WORKDIR}/qcom-opensource/ethernet/neutrino2/standalone/driver/"

EXTRA_OEMAKE =+ "LBITS=32"
EXTRA_OEMAKE =+ "CONFIG_IPA_OFFLOAD=1"

do_install() {
    module_do_install
#    install -d ${D}${sysconfdir}/init.d
#    install -m 0755 ${WORKDIR}/neutrino2_start_stop_le ${D}${sysconfdir}/init.d
#    install -m 0755 ${WORKDIR}/setup_avtp_routing_le ${D}${sysconfdir}/init.d
}

#pkg_postinst_${PN} () {
#    [ -n "$D" ] && OPT="-r $D" || OPT="-s"
#    update-rc.d $OPT -f neutrino2_start_stop_le remove
#    update-rc.d $OPT neutrino2_start_stop_le start 37 S . stop 63 0 1 6 .
#    update-rc.d $OPT -f setup_avtp_routing_le remove
#    update-rc.d $OPT setup_avtp_routing_le start 91 5 . stop 9 0 1 6 .
#}
